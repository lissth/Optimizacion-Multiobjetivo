...
def RestriccionesLambda(x, w1, w2, lam, meta1, meta2):
    m1 = (x[0]**2) + x[1] - w1 * x[2] - meta1
    m2 = (x[1]**2) + x[0] - w2 * x[2] - meta2
    r1 = max(0, m1)
    r2 = max(0, m2)
    return r1 + r2
...
def EvolucionDiferencial(funcion, restricciones, limites, tamPoblacion, CR, iteraciones, w1, w2, lam, meta1, meta2):
    it = 0
    a = 1.0
    ...
    for i, individuo in enumerate(poblacion):
        ap_valores[i] = restricciones(individuo, w1, w2, lam, meta1, meta2)
    ...
    while it < iteraciones and a > 1e-6:
        for i in range(tamPoblacion):
            ...
            aph = restricciones(pobh, w1, w2, lam, meta1, meta2)
            zh = Penalizacion(funcion, aph, pobh)

            app = restricciones(poblacion[i], w1, w2, lam, meta1, meta2)
...
def ObtenMetas(FuncionObjetivo1, FuncionObjetivo2, FunLambda, Restricciones, RestriccionesLambda, limites, tamPoblacion, CR, iteraciones, pesos):
    ...
    tk1, _ = EvolucionDiferencial(FuncionObjetivo1, Restricciones, limites, tamPoblacion, CR, iteraciones, 0, 0, 0, 0, 0)
    tk2, _ = EvolucionDiferencial(FuncionObjetivo2, Restricciones, limites, tamPoblacion, CR, iteraciones, 0, 0, 0, 0, 0)
    ...
    for w in np.linspace(0, 1, pesos):
        ...
        limites_lambda = [limites[0],limites[1], (0, 30)]
        mejorSol, _ = EvolucionDiferencial(FunLambda, RestriccionesLambda, limites_lambda, tamPoblacion, CR, iteraciones, w1, w2, 0, meta1, meta2)
